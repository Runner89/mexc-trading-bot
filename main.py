...
def get_exchange_info():
    return requests.get("https://api.mexc.com/api/v3/exchangeInfo").json()
...
def get_price(symbol):
    ...
    return float(requests.get(url).json().get("price", 0))
...
def get_order_fills(symbol, order_id, api_key, secret):
...
def webhook():
    ...
    if not symbol or not api_key or not secret:
        return jsonify({"error": "symbol/api_key/secret_key fehlt"}), 400
    ...
    offene_position = has_open_sell_limit_order(symbol, api_key, secret)
    ...
    if action == "BUY":
        if not usdt_amount:
            return jsonify({"error": "usdt_amount fehlt", **debug_info}), 400
    ...
    if quantity <= 0:
        return jsonify({"error": "Berechnete Menge ungültig", **debug_info}), 400
    ...
    time.sleep(1)
    fills = get_order_fills(symbol, order_id, api_key, secret)
    executed_price_float = calculate_average_fill_price(fills) if fills else float(order_data.get("price", price))

    if action == "BUY":
        preis_zum_speichern = float(data.get("Preis", executed_price_float))
    ...
    delete_open_limit_sell_orders(symbol, api_key, secret)
    ...
    if quantity > 0 and durchschnittlicher_kaufpreis > 0 and limit_sell_percent is not None:
        ...
        create_limit_sell_order(symbol, quantity, price_rounded, api_key, secret)
    ...
    response_data = {
        ...
        "debug": debug_info,
        "limit_sell_price": round(durchschnittlicher_kaufpreis * (1 + limit_sell_percent / 100), get_price_precision(filters)) if limit_sell_percent else 0,
    }
