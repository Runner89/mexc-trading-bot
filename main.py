...
def SHORT_firebase_speichere_base_order_time(botname, timestamp, firebase_secret)
    try
        ...
        response = requests.put(url, json=data)
        return fBase-Order-Zeit für {botname} gespeichert {timestamp}, Status {response.status_code}
    except Exception as e
...
def SHORT_firebase_loesche_base_order_time(botname, firebase_secret)
    try
        ...
        return fBase-Order-Zeitpunkt für {botname} gelöscht, Status {r.status_code}
    except Exception as e
...
def SHORT_firebase_speichere_ordergroesse(botname, betrag, firebase_secret)
    try
        ...
        return fOrdergröße für {botname} gespeichert {betrag}, Status {r.status_code}
    except Exception as e
...
def SHORT_firebase_speichere_kaufpreis(botname, price, usdt_amount, firebase_secret)
    try
        ...
        if r.status_code == 200
            return fKaufpreis für {botname} erfolgreich gespeichert.
        else
            return fFehler beim Speichern Kaufpreis {r.status_code} {r.text}
    except Exception as e
...
def SHORT_firebase_loesche_kaufpreise(botname, firebase_secret)
    try
        ...
        r = requests.delete(url)
        if r.status_code == 200
            return fKaufpreise für {botname} gelöscht.
        return fFehler beim Löschen Kaufpreise Status {r.status_code}
    except Exception as e
...
def SHORT_sende_telegram_nachricht(botname, text)
    if not TELEGRAM_TOKEN or not TELEGRAM_CHAT_ID
        return Telegram nicht konfiguriert
    full_text = f[{botname}] {text}
    url = fhttpsapi.telegram.orgbot{TELEGRAM_TOKEN}sendMessage
    payload = {chat_id TELEGRAM_CHAT_ID, text full_text}
    try
        r = requests.post(url, json=payload, timeout=10)
        return fTelegram Antwort {r.status_code}
    except Exception as e
...
def SHORT_place_market_order(api_key, secret_key, symbol, usdt_amount, position_side=SHORT)
    ...
    url = f{BASE_URL}{ORDER_ENDPOINT}
    headers = {X-BX-APIKEY api_key, Content-Type applicationjson}
...
def SHORT_get_current_position(api_key, secret_key, symbol, position_side, logs=None)
    ...
    liquidation_price = None
    if logs is not None
        logs.append(fPositions Rohdaten {raw_positions})
    if response.get(code) == 0
        for pos in positions
            if pos.get(symbol) == symbol and pos.get(positionSide, ).upper() == position_side.upper()
                try
                    ...
                    if logs is not None
                        logs.append(fGefundene Position size={position_size}, liqPrice={liquidation_price})
                except (ValueError, TypeError) as e
                    if logs is not None
                        logs.append(fFehler beim Parsen der Position {e})
                    position_size = 0.0
                break
    else
        if logs is not None
            logs.append(fAPI Positions Fehlercode {response.get('code')})
    return position_size, raw_positions, liquidation_price
...
def SHORT_close_all_positions(api_key, secret_key)
    ...
    if positions_resp.get(error)
        sende_telegram_nachricht(BingX Bot, fFehler beim Abrufen der Positionen {positions_resp.get('msg')})
...
def SHORT_close_open_position(api_key, secret_key, symbol, position_side=SHORT)
    ...
    position_size, _, liquidation_price = get_current_position(api_key, secret_key, symbol, position_side, logs=logs)
    if position_size == 0
        logs.append(fKeine offene Position für {symbol} ({position_side}) gefunden.)
        return {code 1, msg Keine offene Position, logs logs}
    ...
    try
        ...
        result = {code -1, msg fFehler beim Parsen der API-Antwort {e}, raw_response resp.text}
    logs.append(fSchließen der Position {result})
    return {result result, logs logs}
