...
def webhook():
    ...
    symbol = symbol_raw.replace("/", "-").upper()  # z.B. BTC/USDT -> BTC-USDT
    side = data.get("side", "BUY").upper()
    amount = float(data.get("usdt_amount", 0))
    ...
    market_resp, market_params, market_raw = place_swap_market_order(symbol, side, amount, api_key, secret_key)

    time.sleep(2)  # kleine Pause, um Position zu erfassen
